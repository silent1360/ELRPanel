...
حالا به آدرس گیتهاب [elr-Worker-Panel](https://github.com/bia-pain-bache/elr-Worker-Panel) میرید و از اون بالا دکمه‌ی Fork رو میزنید.
<br><br>
<p align="center">
...
اینجا روی `elr-Worker-Panel` کلیک کنید تا فعال بشه و `Begin Setup` رو بزنید. مرحله‌ی بعد یه `Project Name` داره که میشه دامین پنل شما، اونو حتما عوض کنید که کلمه‌ی elr توش نباشه و یه اسم دلخواه بذارید وگرنه ممکنه اکانتتون توسط کلادفلر شناسایی بشه.
...
اینجا مثل ورکر تو صفحه قسمت `Bindings` رو پیدا کنید، `Add` بزنید و `KV Namespace` رو انتخاب کنید، `Variable name` باید حتما `elr` باشه (همینجوری که نوشتم) و `KV namespace` اون KV رو انتخاب میکنید که مرحله دو ساختید و `save` میکنید.
...
>[Proxy IP](https://www.nslookup.io/domains/elr.yousef.isegaro.com/dns-records/)
...
> [!TIP]
> اگر خواستید چند Proxy IP داشته باشید میتونید با ویرگول وارد کنید، مثل `151.213.181.145`,`5.163.51.41`,`elr.yousef.isegaro.com`
