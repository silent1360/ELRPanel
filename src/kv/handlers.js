...
export async function getDataset(request, env) {
    ...
    try {
        proxySettings = await env.elr.get("proxySettings", {type: 'json'});
        warpConfigs = await env.elr.get('warpConfigs', {type: 'json'});
    } catch (error) {
        console.log(error);
...
export async function updateDataset (request, env) {
    ...
    if (!isReset) {
        try {
            currentSettings = await env..get("proxySettings", {type: 'json'});
        } catch (error) {
            console.log(error);
            throw new Error(`An error occurred while getting current KV settings - ${error}`);
        }
    } else {
        await env..delete('warpConfigs');
        newSettings = null;
    }
    ...
    try {    
        await env..put("proxySettings", JSON.stringify(proxySettings));          
    } catch (error) {
        console.log(error);
